...
@Slf4j
public class ApprovePaymentTests extends OrdersBaseTest {
    List<MedisendOrderItem> medisendOrderItemsArray = new ArrayList<MedisendOrderItem>();

    @Severity (SeverityLevel.BLOCKER)
    @Description ("APPROVE PAYMENT WITH STATUS UNPAID")
    @Test (groups = { "sanity", "regression" }, priority = 1)
    public void verifyApprovePaymentWithStatusUnpaid() throws IOException {
    ...
    @Severity (SeverityLevel.BLOCKER)
    @Description ("APPROVE PAYMENT WITH STATUS WAITINNG FOR VERIFICATION")
    @Test (groups = { "regression" }, priority = 2)
    public void verifyApprovePaymentWithStatusWaitingforVerification() throws IOException {
        log.info("Running Test : " + Thread.currentThread().getStackTrace()[1].getMethodName());
        ...
        Assert.assertTrue(derawanHelper.validateResponseValueArray(response, SHIPMENTS_PATH + "[0]." + PAYMENTS_PATH, STATUS_PATH,
                STATUS_PAYMENT_PAID, MODE_EQUALS));

        log.info("Exit Test : " + Thread.currentThread().getStackTrace()[1].getMethodName());
    }

    @Severity (SeverityLevel.NORMAL)
    @Description ("APPROVE PAYMENT WITH STATUS PAID")
    @Test (groups = { "regression" }, priority = 3)
    public void verifyApprovePaymentWithStatusPaid() throws IOException {
        log.info("Running Test : " + Thread.currentThread().getStackTrace()[1].getMethodName());
